...
const ArticlePage = () => {
  ...
  const postComment = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASEURL}/articles/${articleData.article._id}/comments/add`,
        {
          method: "POST",
          headers: {
    ...
    <>
          ...
          <div className="drop-down-wrapper">
            ...
            <div className="drop-down-item">
              <button
                type="button"
                onClick={() =>
                  addToBookmarks({ item_id: article._id, item_type: "article" })
                }
              >
      ...
      <div>
        ...
        <main style={{ marginTop: "3.27rem" }}>
          <article className="a-chapter mt-3">
            ...
            <section className="message">
              <p
                id="article-message"
                dangerouslySetInnerHTML={{ __html: article.article_fullText }}
              ></p>
            </section>
          </article>
        </main>
        ...
        <section
          ...
          style={{ paddingInline: "2ch", marginTop: "2ch" }}
        >
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="author-name">
                        <span
                          style={{ fontSize: "0.87rem", fontFamily: "Poppins" }}
                        >
                          {comment.username || "Anonymous"}
                        </span>
                        <br />
                        <small
                          style={{ fontSize: "0.67rem", fontFamily: "Poppins" }}
                        >
                          {`${formatDate(new Date(comment.date).toISOString(), "PPP")}`}
                        </small>
                      </div>
                    </div>
                    {comment.username === getUserNameFromLocalStorage() ? (
                      <div className="comment-actions">
                        <button
                          onClick={async () => {
                            const response = await fetch(
                              `${BASEURL}/articles/${articleData.article._id}/comments/${comment.date}/delete?user=${comment.username}`,
                              {
                                method: "DELETE",
                              },
                            );
